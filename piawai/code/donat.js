...
button:{data:[
{id:1,nama:"Insert",icon:"plus",url:"d.service.create()"},
...
<button class="modalclose kanan merah " onclick="tutup()">x</button>
</div>
...
for(i in data){
out+=`<li class="${css.li}">
<a class="${css.a}" onclick="d.service.read('${data[i].url}')"> ${svg.icon(data[i].icon)} ${data[i].nama}</a>
</li>`;}
out+=`</ul>`;
return out;
},
...
aksi:function(i){
return `
<button data-action="Edit" class="${css.button}" onclick="d.controller.edit(${i})"> ${svg.icon('pen')} <span>Edit</span></button>
`;
},
...
out+=`<button data-action="${nama}" class="${css.button}  ${getWarna(nama)}" onclick="${url}"> ${svg.icon(icon)} ${nama}</button>`;}
out+=`</div>`;
return out;
},
...
out+=`<div class="col-1-${step} card" onclick="${url}" >
...
d.model.edit.induk=table.data[i].id; // dev
